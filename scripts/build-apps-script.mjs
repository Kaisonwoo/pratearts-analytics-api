...
async function sources(root, folder = 'src') {
  ...
  const lists = await Promise.all(entries.map(entry => {
    ...
    return entry.isDirectory() ? sources(root, relative) : [relative];
  }));
  return lists.flat().filter(file => file.endsWith('.gs')).sort();
}
...
export async function buildRuntime(root, revision = 'uncommitted') {
  ...
  const chunks = await Promise.all(files.map(async file => ({ file, content: await readFile(path.join(root, file), 'utf8') })));
  const manifest = await readFile(path.join(root, 'src/appsscript.json'), 'utf8');
  JSON.parse(manifest);
  const sourceHash = createHash('sha256').update(JSON.stringify([chunks, manifest])).digest('hex');
  const code = `// Pratearts Analytics API | revision: ${revision}\n// Source SHA-256: ${sourceHash}\n` +
    chunks.map(({ file, content }) => `\n// BEGIN ${file}\n${content}\n// END ${file}\n`).join('');
  new vm.Script(code, { filename: 'Runtime.gs' });
  return { code, manifest, files, sourceHash, revision };
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(JSON.stringify({ revision: result.revision, modules: result.files.length, sourceHash: result.sourceHash }));
}
